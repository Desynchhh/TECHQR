<?php
	class Events extends CI_Controller{
		...
		// Resets everything in the event. Empties all teams, resets scores, deletes message and actions, and removes teams logged answers, so they can answer the same assignments again.
		public function reset($e_id, $fullReset = 0){
			// Check user is logged in
			if(!$this->session->userdata('logged_in')) {
				redirect('login');
			}
			...
			foreach($teams as $team){
				...
				// Keep students in their teams unless $fullReset is true
				if($fullReset == 1) {
		...
		public function open_pdf($eventfolder, $path_location){
			...
			// Set the browser to be able to read/view a PDF file
